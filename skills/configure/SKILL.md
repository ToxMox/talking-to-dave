...
7. Walk the user through the choices, showing current values, batched into few questions and honoring the CURRENT dialog policy while asking:
   ...
   - `queue`: `widgets` | `text` | `off` (question-queue mode: told-once asks backed by a session-scoped file queue with a linked names line; `text`, the default, keeps typed answers only; `widgets` adds the collapsible answer panel at a real context cost, roughly 600-700 tokens of bar markup re-emitted on every substantive reply while any question is open, so say what it costs when offering it; `off` restores the classic restated footer with waiting counts).
