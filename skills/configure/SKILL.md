...
1. Resolve the data directory: `$CLAUDE_PLUGIN_DATA` if set AND its final path segment starts with `talking-to-dave` (the variable can leak from other plugins' contexts into the session shell), else `~/.claude/plugins/data/talking-to-dave-talking-to-dave`. Read `config.json` there if it exists and use it as the starting point.
