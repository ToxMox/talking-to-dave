...
var CONTRACT_REV="2026-08-11";
  ...
  function contractBody(o){
    ...
    if(qmode!=="off"){
      ...
      if(qmode==="widgets"){
        rules.push(qhead+"From then on every substantive reply ends with two devices, the names line then the bar. "+qnames+"The bar is a widget: one collapsed line that unfolds in place, no message sent, into the answer panel, whose rows (same order) carry the ID with its short name, a one-line gist, answer buttons with \"(my pick)\" on the recommendation, and a dim drop control; every button's inserted text ends with a period and a space, so several clicks compose one clean line. An empty queue renders neither device, and typing \"q\" summons a fresh panel when no bar is at hand. The bar renders below the reply mechanically: its widget call shares the reply's assistant message, after the text block, and anything after the bar is at most one minimal closing line. Where inline widgets do not exist the bar drops, the names line gains \"(q to view)\", and the panel is decision tables."+qtail);
      }else{
  ...
  function buildChatPreferences(o){
    ...
    if(o.ids){
      p.push("When you mention an ID or numbered item (Q1, T-1043, an issue number), carry its short human name with it rather than the bare number, so I never scroll back to remember what it was.");
    }
    if(o.visual){
